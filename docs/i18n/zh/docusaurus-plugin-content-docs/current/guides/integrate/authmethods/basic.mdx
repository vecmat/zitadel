...
发送您的 `client_id` 和 `client_secret` 作为基本认证头. 请注意，OAuth2 要求客户端_id 和 client_secret 格式编码。 所以检查 [客户端基本认证方法](/docs/apis/openidoauth/authn-methods#client-secret-basic) 以便如何正确地构建它。
...
--data code=${code} \
--data rediredirectt_uri=${redirect_uri}
```
