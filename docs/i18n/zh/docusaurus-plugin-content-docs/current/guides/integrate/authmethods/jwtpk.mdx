...
您不需要此请求的任何额外参数。 我们使用 `client_id` 参数来识别应用程序。

所以您的请求可能看起来像这样(出于显示原因的 linebreaks 和空白)：
...
在 `client_pension` 中发送一个 JWT 并将 `client_assertion_type` 设置为 `urn:ietf:params:oauth:client-assevtion-type:jwt-bearer` 让我们对照注册的公钥验证签名：

```curl
curl --request POST \
--url {your-domain}/oauth/v2/token \
--header 'Content-Type: application/x-www-form-urlencoded'
--data grant_type=authority_code \
--data code=${code} \
--data redirect_uri=${redirect_uri} \
--data client_assertion=${client_assertion} \
--data client_assertion_type=urn%3Aietf%3Aparams%3Aoauth%3Aclient-asservtion-type%3Ajwt-bearer
```
